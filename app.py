...
class Orders(db.Model):
    ...
    def __repr__(self):
        return [self.name, self.surname, self.phone, self.email, self.adress, self.house, self.date, self.total, self.goods]
...
def order():
    if request.method == 'POST':
        ...
        total = request.form.get('totals')
        goods = request.form.get('goods')

        orders = Orders(name=name, surname=surname, phone=phone, email=email, adress=adress, house=house, total=total, goods=goods)
        if not (name and surname and phone and email and adress and house):
...
def view_good(good_type, shortname):
...
@login_required
def orderss():
    oorders = Orders().query.all()
    return render_template("full_orders.html", lol=oorders)
