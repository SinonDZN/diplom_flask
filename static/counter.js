...
window.addEventListener('click', function (event) {
        ...
        if (event.target.dataset.action === 'plus') {
            ...
            const wops = document.querySelector('.tbh').outerHTML
            const wopss = document.querySelector('.card-titles').innerText
            localStorage.setItem('div', wops)
            localStorage.setItem('name', wopss)
        ...
        if (event.target.dataset.action === 'minus') {
            ...
            if (parseInt(divcount.innerText) > 1) {
                ...
                const wopsa = document.querySelector('.card-titles').innerText;
                const ops = document.querySelector('.tbh').outerHTML;
                localStorage.setItem('div', ops)
                localStorage.setItem('name', wopsa)
